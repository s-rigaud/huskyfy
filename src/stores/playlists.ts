...
const COMMON_GENRES = ([
  ...
  'mellow', 'drill', 'house', 'indie', 'psych', 'swing', 'metal', 'lo-fi', 'dance',
  'rock', 'jazz', 'trap', 'soul', 'funk', 'pop', 'r&b', 'rap', 'edm'
] as const)
...
export const usePlaylistsStore = defineStore('playlists', {
  ...
  actions: {
    ...
    async reorderTracks(playlistId: string, sortedTracks: SpotifyTrack[]) {
      const snapshotId = await api.spotify.playlists.reorderTracks(
        playlistId, sortedTracks, this.playlists[playlistId].snapshot_id
      )
      this.playlists[playlistId].snapshot_id = snapshotId
    ...
    async sortPlaylistTracksByGenres(playlistId: string) {
      ...
      // 3.0 Update playlist
      await this.reorderTracks(playlistId, sortedTracks)
    },
